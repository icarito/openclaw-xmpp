...
export async function decryptOmemoMessage(
  ...
  try {
    ...
    if (isV2 && keyElements.length === 0) {
      for (const keysEl of header.getChildren("keys")) {
        ...
        ourKeyElement = keyElements.find(
          (k: Element) => parseInt(k.attrs?.rid, 10) === ourDeviceId
        );
    ...
    if (!ourKeyElement) {
      log?.debug?.(`[${accountId}] OMEMO message not encrypted for our device ${ourDeviceId} (found keys for: ${keyElements.map((k: Element) => k.attrs?.rid).join(", ")})`);
      return null;
    }
